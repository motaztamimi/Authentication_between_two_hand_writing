...
class TriplteLoss(nn.Module):
    ...
    def forward(self, anc, pos, neg):
        d_a_p = torch.pairwise_distance(anc, pos)
        d_a_n = torch.pairwise_distance(anc, neg)

        loss = torch.sum(torch.max(d_a_p - d_a_n + self.margin, torch.tensor(0.)))
        return loss
...
def triplet_train( model, loss_function, optimizer,train_loader,loss_history, epoch):
    ...
    # loss_history_for_ephoces.append( sum(batches_loss) / len(batches_loss))
        print(sum(batches_loss) / len(batches_loss))

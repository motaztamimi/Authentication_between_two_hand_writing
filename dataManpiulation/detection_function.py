...
def union_left_ctr(cur_ctr, next_ctr, canvas):
    ...
    ret, temp_canvas = cv2.threshold(
        temp_canvas, 109, 255, cv2.THRESH_BINARY_INV)
...
def find_letters(line_image):
    ...
    if line_image.shape[0] < 40:
        line_image = cv2.resize(
            line_image, (line_image.shape[1] * 2, line_image.shape[0] * 2))
    ...
    if cv2.__version__.startswith('3.'):
        ...
        (ctrs, __) = cv2.findContours(thresh.copy(),
                                      cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    # sort contours
    sorted_ctrs = sorted(
        ctrs, key=lambda ctr: cv2.boundingRect(ctr)[0], reverse=True)
        ...
        if h > 3:
            ...
            if i < length - 1 and new_ctr[i].x_start >= new_ctr[i + 1].x_start and new_ctr[i].x_end <= new_ctr[
                ...
                if union_left_ctr(new_ctr[i], new_ctr[i + 1], canvas) == 0:
                    ...
                    roi = canvas[new_ctr[i + 1].y_start:Y_end_bigger,
                                 new_ctr[i + 1].x_start:new_ctr[i + 1].x_end]
                    roiriginal = line_image[new_ctr[i + 1].y_start:Y_end_bigger,
                                            new_ctr[i + 1].x_start:new_ctr[i + 1].x_end]
                    i += 1
            else:
                roi = canvas[y:y + h, x:x + w]
            ...
            letter = np.pad(roiriginal, pad_width=10,
